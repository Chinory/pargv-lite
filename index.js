 ...
 * @typedef {{ [nestable: string]: { bool: boolean, str: string, num: number, list: Text[] } }} Prop supprt `-Xo1.o2.o3 val`, `-Xo1.o2=val` `--opt=o1=val`
 * 
 ...
 * @property {OptKit} [str] External Options to set a string, can escape the quotes
 * @property {OptKit} [num] External Options to set `Number(arg)`. Raise `err` with `NaN`. Provide a JSON Number Type in other language, naturally
 * @property {OptKit} [nul] Options to set `null`, that is, add a `null` to the array. (what??) maybe the user wants to cause some NullExceptions
 * @property {OptKit} [rst] Options to set the array back to ***def***: `_`
 * 
 * @typedef {Object} PropKit
 * @property {Prop} _
 * @property {OptKit} [set] Options to set the raw string slice from the argv. Raise `err` if that is not a `string`
 * @property {OptKit} [str] External Options to set a string, can escape the quotes. Raise `err` if is not a `string`
 * @property {OptKit} [num] External Options to set `Number(arg)`. Raise `err` if is not a `number`. Raise `err` with `NaN`. Provide a JSON Number Type in other language, naturally
 * @property {OptKit} [not] Options to set !***def***. Raise `err` if is not a `boolean`
 * @property {OptKit} [inv] Options to set `!cur`. Raise `err` if is not a `boolean`
 * @property {OptKit} [yes] Options to set `true`. Raise `err` if is not a `boolean`
 * @property {OptKit} [no]  Options to set `false`. Raise `err` if is not a `boolean`
 * @property {OptKit} [on]  Options to set `true`. Raise `err` if is not a `boolean`
 * @property {OptKit} [off] Options to set `false`. Raise `err` if is not a `boolean`
 * @property {OptKit} [nul] Options to set `null`, or add a `null` to the array.
 * @property {OptKit} [rst] Options to set the value/array in that postion back to ***def***: `_`
 * 
 * @typedef {OptDef | OptDef[]} ExitKit there are only two types of container in JSON: list, dict. And this the list one
 * @typedef {BoolKit | TextKit | ListKit | PropKit} VarKit
 * @typedef {{ _?: VarVal, [act: string]: OptKit }} VarKitLike
 ...
 * 
 * @typedef {Bool | Text | List | Prop} VarVal // `undefined` is from `Bool`, `null` is from `Text`
 * @typedef {{ [key: Key]: VarVal }} VarValMap The `req` arg of `parse`.
 * @typedef {{ i: number, key: Key, opt: Opt }} ExitVal 
 * 
 * @callback IsFatal
 * @param {{ i: number, msg: string, opt: Opt, key?: Key, val?: VarVal, prop?: Prop }} err
