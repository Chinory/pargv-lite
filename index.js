...
module.exports = function parse (argv, i, module, moduleName, moduleModel, callback) {
	var optSet = {}, optRst = {}, opt, optUndone, key, keyUnnamed, value, def, opts, j;
	for (key in moduleModel) {
		...
		if (Array.isArray(def)) {
			...
			if ((opts = value.rst)) for (j = 0; j < opts.length; ++j) if ((opt = opts[j]) !== "--") optRst[opt] = key;
		...
		if (optUndone) {
			...
			if (~eq) {
				...
				if ((key = optSet[opt])) {
					...
					if (!callback("can not set value of rst option", opt, module, moduleName, moduleModel)) return;
				} else if (keyUnnamed) {
					module[keyUnnamed].push(arg);
				} else if (!callback("uncaptured argument", arg, module, moduleName, moduleModel)) {
					return;
